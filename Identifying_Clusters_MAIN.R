...
# load auxillary functions for analysis 
...
Classification.fn = function(x,y,m_0){
  ...
  for(w in (K-1):2){ # classification for the number of cluster is from K-1 to 2
    ...
    c_list = c(which(colSums(INF)>=m_0,arr.ind=TRUE))    # collect the number of the group whose size is greater and equal t0 m_0
    ...
    while(diff > exp(-10) && c_list_l > 0 && cri != 1){ # the criteria for iterating non-hierarchical classification
      ...
      for (s1 in c_list){    # the s1-cluster
        re_1 = which(c(grp0)==s1,arr.ind=TRUE)   # position
        
        for (s2 in re_1){     # the s2 position
          grp = c(grp0)
          grp[s2] = 0      # let one of robot in s1-cluster become group 0 
