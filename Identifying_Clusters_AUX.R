...
# Estimate robot strengths for a given cluster of robots
beta_esti.fn = function(grp_info){
  ...
  xc = x0%*%grp_info 
  xcn = xc[ ,1:(num_grp-1)]-xc[ ,num_grp]%*%t(w_clu) # constraint: sum of beta = 0
...
# estimated three types of MSPE (1-MSPE_D, MSPE_P, MSPE_Y) by removing one out cross-validation
...
# prediction of the playoff based on selected optimal models
pred.fn = function(beta_opt){
  ...
  # predict which alliances will advance to semifinials
  semi_y = y0_po[1:12,]
  semi_x = x0_po[1:12,]
  ...
  # predict which alliances will advance to the finial
  final_y = y0_po[13:18,]
  final_x = x0_po[13:18,]
  ...
  # predict which alliance will win the championship
  champ_y = y0_po[19:21,]
  champ_x = x0_po[19:21,]
